...
def click(event):
    ...
    """
    global x, y, r, point
    if (c.coords(ball1)[1]) <= event.y <= (c.coords(ball1)[3]):
        if (c.coords(ball1)[0]) <= event.x <= (c.coords(ball1)[2]):
            point += 1
            c.delete(ALL)
            lbl2.configure(text=point)
    if (c.coords(ball2)[1]) <= event.y <= (c.coords(ball2)[3]):
        if (c.coords(ball2)[0]) <= event.x <= (c.coords(ball2)[2]):
            point += 1
            c.delete(ALL)
            lbl2.configure(text=point)
